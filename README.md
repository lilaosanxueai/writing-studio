...
| ✍️ 文案 | **顶尖文案工艺**：通用文笔铁律（具体压倒抽象/长短节奏/虚词清场/禁升华结尾/禁词表）+ 8 种文体各自的手法手册；**风格库**（蒸馏自代表性博主的公开手法，学手法不搬句子，下拉分组展示）：图文博主——咪蒙式·情绪爆款、九边式·深度长文；知识博主——罗翔式·案例思辨、罗振宇式·知识口播；另有自定义风格描述入口。每张风格卡标注最适合的文体并内嵌价值观护栏。；两段式管道（初稿 → 💎 编辑打磨二遍稿：砍 AI 味、抽象换画面、锻新金句），生成后可自动或手动打磨；8 种格式 × 5 种语气 × 3 档长度；话题分析推荐格式一键套用；素材精选勾选必用灵感；流式生成、指令改写、手工编辑、复制/下载/存飞书 |
